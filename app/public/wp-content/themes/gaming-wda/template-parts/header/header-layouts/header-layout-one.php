<div class="wpdevart-header-container">
        ...
        <div class="wpdevart-main-header-section-mobile-logo">
            <?php 
                if ( has_custom_logo() && is_front_page()) { ?>
                    <h1 class="mobile-image-logo-resizing wpdevart-header-logo-spaces"><a href="<?php echo esc_url(home_url()) ?>"><?php the_custom_logo() ?></a></h1>
                <?php }
                else if ( has_custom_logo() && !is_front_page()) { ?>
                    <h2 class="mobile-image-logo-resizing wpdevart-header-logo-spaces"><a href="<?php echo esc_url(home_url()) ?>"><?php the_custom_logo() ?></a></h2>
                <?php }
                else if ( !has_custom_logo() && is_front_page() && get_theme_mod( 'wpdevart_gaming_wda_header_logo_gradient_color' )) { ?>
                    <h1 class="wpdevart-logo-text-gradient"><a href="<?php echo esc_url(home_url()) ?>"><?php echo get_bloginfo('name') ?></a></h1>
                <?php }
                else if ( !has_custom_logo() && is_front_page() && empty(get_theme_mod( 'wpdevart_gaming_wda_header_logo_gradient_color' ))) { ?>
                    <h1 class="wpdevart-logo-text"><a href="<?php echo esc_url(home_url()) ?>" class="wpdevart-logo-text-simple"><?php echo get_bloginfo('name') ?></a></h1>
                <?php } 
                else if ( !has_custom_logo() && !is_front_page() && get_theme_mod( 'wpdevart_gaming_wda_header_logo_gradient_color' )) { ?>
                    <h2 class="wpdevart-logo-text-gradient"><a href="<?php echo esc_url(home_url()) ?>"><?php echo get_bloginfo('name') ?></a></h2>
                <?php }
                else if ( !has_custom_logo() && !is_front_page() && empty(get_theme_mod( 'wpdevart_gaming_wda_header_logo_gradient_color' ))) { ?>
                    <h2 class="wpdevart-logo-text"><a href="<?php echo esc_url(home_url()) ?>" class="wpdevart-logo-text-simple"><?php echo get_bloginfo('name') ?></a></h2>
                <?php }
            ?>
        </div>
        <?php if ( (get_theme_mod('header_text') != 0) && (get_bloginfo('description') != '') ) { ?>
            <p class="wpdevart-site-description"><?php echo get_bloginfo('description') ?></p><?php 
        } ?>
    </div>
    ...
    <div role="navigation" class="<?php if (get_theme_mod( 'wpdevart_gaming_wda_header_show_action_button', '1' )) { echo esc_attr('nav-container-without-action-button'); } else { echo esc_attr('nav-container-layout-one'); } ?>">
    ...
    <?php if ( class_exists( 'WooCommerce' ) ) { ?>
            ...
            <div class="header-cart">
                <a class="cart-customlocation" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'gaming-wda' ); ?>">
				...
				<?php wpdevart_gaming_wda_header_cart_layout_one() ?>
            </div>   
    <?php } ?>
